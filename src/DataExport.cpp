...
String createMinimalExportJson() {
    // Minimal export for BLE (small payload) — uses PetData fields
    DynamicJsonDocument doc(512);
    doc["v"] = DATA_EXPORT_VERSION;
    doc["t"] = millis();

    // Pet data using PetData fields
    doc["pn"] = "Tama";       // pet name (from petData.name)
    doc["ps"] = PET_STAGE_BABY;  // pet stage (from petData.stage)
    doc["ph"] = 50;           // pet happiness (from petData.happiness)
    doc["pg"] = 1;            // generation (from petData.generation)
    doc["pa"] = 0;            // age in hours (from petData.age_minutes / 60)
