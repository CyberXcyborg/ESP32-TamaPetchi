...
- [ ] Create branch: `git checkout -b feature/phase14-v1.4 develop`
- [ ] Verify clean build: `pio run -e esp32dev` — zero errors
- [ ] Verify baseline tests: `pio test -e native` — 145/152 pass (7 known failures)
...
- [ ] Run `pio test -e native` — target: 152/152 pass
- [ ] Commit: fix(tests): resolve 7 pre-existing test failures in backup/restore and achievements

### 14.2 — OTA Rollback Support
- [ ] Research ESP32 OTA partition scheme (ota_0, ota_1, otadata)
- [ ] Add OTA_Rollback module: OTA_Rollback.h / OTA_Rollback.cpp
...
- [ ] Add rollback status to GET /api/system/info response
- [ ] Update platformio.ini to use OTA partition scheme if not already
- [ ] Verify compilation: `pio run -e esp32dev` — zero errors
- [ ] Add unit tests for rollback state machine
- [ ] Commit: feat(ota): add dual-partition OTA rollback with auto-fallback

### 14.3 — Pet Trading via MQTT
- [ ] Add Trading module: Trading.h / Trading.cpp
- [ ] Define MQTT topic scheme: tamapetchi/trade/{device_id}/offer, /accept, /reject
...
- [ ] Add POST /api/pets/{id}/trade HTTP endpoint to initiate trade
...
- [ ] Commit: feat(trading): MQTT-based pet trading between TamaPetchi devices

### 14.4 — Sound Pack System
- [ ] Add SoundPack module: SoundPack.h / SoundPack.cpp
...
- [ ] Create alternate sound pack: data/sounds/chirp.json (higher-pitched, shorter notes)
- [ ] Add GET /api/settings/sound-packs — list available packs
- [ ] Add POST /api/settings/sound-pack — select active pack
- [ ] Add POST /api/settings/sound-pack/upload — upload custom pack (max 4KB)
...
- [ ] Commit: feat(sound): user-customizable sound packs with web UI upload
...
- [ ] Commit: feat(ui): add trading and sound pack management to web UI

### 14.6 — Community & Developer Tools
- [ ] Add CONTRIBUTING.md: build instructions, code style, PR process, testing requirements
- [ ] Add GitHub issue templates: bug_report.md, feature_request.md
- [ ] Add scripts/flash-batch.py: batch flash multiple ESP32 devices (extend existing provisioning script)
- [ ] Add scripts/simulate-24h.py: simulate 24h of pet activity in native test, verify no heap growth
- [ ] Update README.md with Phase 14 features
- [ ] Update CHANGELOG.md with Phase 14 changes
- [ ] Update PROJECT_STATUS.md
- [ ] Commit: docs(community): add CONTRIBUTING.md, issue templates, batch flash script
...
- [ ] Update PROJECT_STATUS.md
- [ ] Create git tag: git tag v1.4.0
- [ ] Merge: git checkout develop && git merge feature/phase14-v1.4
- [ ] Merge: git checkout main && git merge develop
- [ ] Push all branches: git push origin main develop --tags
- [ ] Create GitHub release with firmware.bin attached
- [ ] Commit: chore(release): v1.4.0 — OTA rollback, pet trading, sound packs, test fixes
